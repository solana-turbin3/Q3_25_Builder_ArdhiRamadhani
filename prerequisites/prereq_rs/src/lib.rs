#[cfg(test)] mod tests{
    use solana_sdk::{instruction::Instruction, message::Message, signature::{ read_keypair_file, Keypair, Signer}, system_program, transaction::Transaction};
    use bs58;
    use std::io::{self,BufRead};
    use solana_client::{nonblocking::rpc_client, rpc_client::RpcClient};
    use solana_program::{pubkey::Pubkey, system_instruction::transfer, hash::hash, instruction::AccountMeta};
    use std::str::FromStr;  


    const RPC_URL: &str = "https://turbine-solanad-4cde.devnet.rpcpool.com/9a9da9cf-6db1-47dc-839a-55aca5c9c80a";
    const RPC_URL_AIRDROP: &str = "https://api.devnet.solana.com";
    ...
    fn airdrop(){
        // import our keypair
        let keypair = read_keypair_file("devnet2-wallet.json").expect("Couldn't find  wallet file");
        
        // we'll establish a connection to Solana devnet using the const we defined above
        let client = RpcClient::new(RPC_URL_AIRDROP);
    ...
    fn transfer_sol(){
        ...
        // let keypair2 = Keypair::from_bytes(&secret_key_bytes)
        // .expect("Invalid keypair bytes");
        let keypair2 = read_keypair_file("devnet-wallet.json").expect("Couldn't find wallet file");
    ...
    fn transfer_all_sol(){
        ...
        let to_pubkey = Pubkey::from_str("38WNVZkpovhyRqYDqpXomwDAbgXzRcLtV5ZpyQMPSneL").unwrap();
    ...
    fn submit_rs(){
        let rpc_client = RpcClient::new(RPC_URL);
        let signer = read_keypair_file("devnet1-base58-wallet.json").expect("Couldn't find wallet file");
